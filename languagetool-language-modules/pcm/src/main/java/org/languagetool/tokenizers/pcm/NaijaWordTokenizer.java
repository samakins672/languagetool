 ...
 * USA
 */
package org.languagetool.tokenizers.pcm;
 ...
 * @since 2.5
 */
public class NaijaWordTokenizer extends WordTokenizer {
  ...
  /**
   * Tokenizes text. The Naija tokenizer differs from the standard one in two
   * respects:
   * <ol>
    ...
    if (!s.isEmpty()) {
      ...
      if (!s.isEmpty()) {
        if (!s.contains("-") && !s.contains("'") && !s.contains("’")) {
          ...
          if (NaijaTagger.INSTANCE.tag(Arrays.asList(normalized)).get(0)
            .isTagged()) {
            l.add(s);
          }
